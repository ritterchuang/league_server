...
class BoardGtrFishRandomTargetTest {
    BoardGtrFishRandomTarget boardGtr;

    @BeforeEach
    void setUp() {
        // 1. 準備建構參數
        BoardGtrConfig boardGtrConfig = new BoardGtrConfigFactory().create(
                ConstPlayGameField.PlayGameFieldResource.R201_JCBY_JAVA_ONE_TARGET,
                ConstBoardGtr.BoardGtrControlAlgorithmType.NATURE);

        // 2. 建構局產生器
        this.boardGtr = new BoardGtrFishRandomTarget(
