package org.lsj.gs.mathBoardGtr.games.cjnn_java;
...
// 抽象新超級牛牛局產生器
public abstract class AbstractBoardGtrBaiRenCjnnJava extends AbstractBoardGtrBaiRen {
    protected TableCjnnJava table; // 牌桌
    protected final GameTypeConfigExtendBaiRen gameTypeConfigExtend; // 客製遊戲設定

    public AbstractBoardGtrBaiRenCjnnJava(GamePlayerSimulation player, PoolCtr poolCtr, PlayGameFieldConfig playGameFieldConfig, ControlAlgorithmConfig controlAlgorithmConfig) {
        // 1. 初始化
        ...
        // 2. 產生牌桌
        try {
            this.table = new TableCjnnJava(
                    lastTableId.getAndIncrement(),
                    playGameFieldConfig.getTableFieldConfig(),
                    (TableGameConfigCjnnJava) new TableGameConfigFactory().createTableGameConfig(playGameFieldConfig.getTableFieldConfig().getGameId(), player.getUser()),
    ...
    private void onRecvBetMsg(int randomAreaId, int randomBet){
        ...
        // 2. 正常下注
        ((StateGameBeginCjnnJava)this.table.getStateMgr().getStateMap().get(BaiRenGameState.GAME_BEGIN.getCode())).onRecvBetMsg(
    ...
    private int calculateRandomAreaId(){
        return super.tableUtil.getNotSupportSetRngDataRandomUtil().getRandomElement(
                Arrays.stream(ConstMathNiu.BetAreaEnum.values())
                        .filter(betAreaEnum -> betAreaEnum.getCode() != ConstMathNiu.BetAreaEnum.INVALID.getCode()
