...
import com.lx.gs.math.games.bjl_java.entity.message.StcChartBjlJava;
import com.lx.gs.math.games.bjl_java.entity.message.StcCutReturnBjlJava;
import com.lx.gs.math.games.bjl_java.module.chartCtr.ChartCtrBjlJava;
import com.lx.gs.math.games.bjl_java.module.gameAdjust.GameAdjustBjlJava;
import com.lx.gs.math.games.bjl_java.module.logic.LogicBjlJava;
import com.lx.gs.math.games.bjl_java.module.robotLogic.RobotLogicBjlJava;
import com.lx.gs.math.games.bjl_java.module.state.StateGameBeginBjlJava;
import com.lx.gs.math.games.bjl_java.module.state.StateGameEndBjlJava;
import com.lx.gs.math.games.bjl_java.module.state.StateWaitBeginBjlJava;
...
// 新百家樂遊戲桌
public class TableBjlJava extends AbstractTableMessageBaiRen {
    private static final Logger LOG = LoggerFactory.getLogger(TableBjlJava.class);

    private final ChartCtrBjlJava chartCtr; // 路圖計算器
    private final LogicBjlJava logic; // 遊戲邏輯
    private final RobotLogicBjlJava robotLogic; // 機器人遊戲邏輯
    private final GameAdjustBjlJava gameAdjust; // 遊戲調控

    public TableBjlJava(int tableId,
        ...
        super(  tableFieldConfig,
                ...
                user,
                tableUtil);
        this.chartCtr = new ChartCtrBjlJava(tableUtil);
        this.logic = new LogicBjlJava(this,
                tableFieldConfig,
                tableGameConfig,
                ...
                new GameBetLogResultCtrBaiRen(
                        ...
                        tableUtil),
                tableUtil);
        this.robotLogic = new RobotLogicBjlJava(
                super.getGamePlayerHlr(),
                super.getPoolCtr(),
                tableUtil,
                this.logic);
        this.gameAdjust = new GameAdjustBjlJava(
    ...
    public void updateChart(int[] winBetAreaArray){
        this.chartCtr.updateChart(winBetAreaArray);
    }
    ...
    public void sendChart(){
        if(super.getGamePlayerHlr().isHumanEnterTable()) {
            StcChartBjlJava stcChart = new StcChartBjlJava(this.chartCtr.getChartArray());
    ...
    // 斷線重連資訊
    private StcCutReturnBjlJava calculateCutReturn(double leftTimeSec){
        // 1. 計算當前狀態
        ...
        // 5. 取得閒牌
        int[] playCards = this.logic.getPlayCardArray();

        // 6. 計算黑牌
        int[] bankCards = this.logic.getBankCardArray();

        // 7. 計算牌型
        int[] compareScore = this.logic.calculatePlayBankPointArray();

        return new StcCutReturnBjlJava(stage, leftTime, areas, luckyStarAreas, playCards, bankCards, compareScore);
    }
    ...
    public void registerState(){
        // 1. 註冊狀態機
        super.getStateMgr().registerState(new StateWaitBeginBjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.WAIT_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameBeginBjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameEndBjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_END.getCode()));
    ...
    private void receivedGameCommandBet(ReceiveGameCommand receiveGameCommand){
        ...
        // 2. 請求
        try {
            ((StateGameBeginBjlJava)super.stateMgr.getStateMap().get(BaiRenGameState.GAME_BEGIN.getCode()))
