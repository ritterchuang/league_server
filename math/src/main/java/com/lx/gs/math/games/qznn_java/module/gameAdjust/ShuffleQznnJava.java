package com.lx.gs.math.games.qznn_java.module.gameAdjust;
...
import com.lx.gs.math.games.qznn_java.entity.PreGameAdjustResultQznnJava;
...
// 搶莊牛牛發牌
public class ShuffleQznnJava extends AbstractShuffle implements IShuffle {
    private final LogicQznnJava logic; // 邏輯
    private final int DEAL_COUNT_PER_PLAYER = 5; // 每人發牌數

    public ShuffleQznnJava(AlgorithmTypeCtr algorithmTypeCtr, LogicQznnJava logicQZNN, ITableUtil tableUtil) {
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 5. 封裝
        return new PreGameAdjustResultQznnJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultQznnJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
    ...
    public GameAdjustResult calculateGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 計算所有玩家輸贏結果
        Map<Integer, UidScore> uidScoreMap = this.logic.calculateUidScoreMap(((PreGameAdjustResultQznnJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap());
