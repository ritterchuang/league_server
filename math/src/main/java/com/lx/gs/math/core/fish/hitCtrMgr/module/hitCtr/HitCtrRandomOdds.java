...
import com.lx.gs.math.core.fish.hitCtrMgr.module.hitCtr.hitCtrUtil.module.HitCtrUtilRandomOdds;

// 隨機倍數打擊計算器
public class HitCtrRandomOdds extends AbstractHitCtr implements IHitCtr {
    private final ConstMathFish.HitType hitType; // 打擊類型
    private final HitTypeConfigExtendRandomOdds config; // 客製打擊計算器設定
    private final HitCtrUtilRandomOdds hitCtrUtil; // 打擊計算工具包

    public HitCtrRandomOdds(HitCtrConfig hitCtrConfig, GamePlayerHlr gamePlayerHlr, PoolCtr poolCtr, ITableUtil tableUtil) {
        super(hitCtrConfig, gamePlayerHlr, poolCtr, tableUtil);
        this.hitType = hitCtrConfig.getHitType();
        this.config = (HitTypeConfigExtendRandomOdds) hitCtrConfig.getHitTypeConfigExtend();
        this.hitCtrUtil = new HitCtrUtilRandomOdds(this.config, tableUtil);
    }
