...
import com.lx.gs.math.games.sbxc_java.entity.result.roundResult.RoundResultExtendSbxcBaseGame;

// 客製局結果包裝者一般 三倍小丑基礎遊戲
public class RoundResultNormalPgrSbxcBaseGame extends AbstractRoundResultNormalPgr {
    private final int reSpinColumnIndex = 2;

    public RoundResultNormalPgrSbxcBaseGame(ClientRoundResultPgrConfig config, ITableUtil tableUtil) {
    ...
    public RoundResultExtend packageRoundResultExtend(RoundHlrResultNormal roundHlrResultNormal) {
        ...
        // 2. 轉型777運算結果
        RoundHlrResultExtendSbxcBaseGame roundHlrResultExtendSbxcBaseGame = (RoundHlrResultExtendSbxcBaseGame) (roundHlrResultNormal.getRoundHlrResultExtend());

        // 3. 計算客端重轉畫面
        ScreenResult clientReSpinScreenResult = this.clientScreenPgr.calculateScreenResult(roundHlrResultExtendSbxcBaseGame.getScreenGtrResult());

        // 4. 計算客端重轉算分
        GameResult clientGameResult = super.packageGameResult(clientReSpinScreenResult, roundHlrResultExtendSbxcBaseGame.getReSpinGameCtrResult());

        // 5. 回傳
        return new RoundResultExtendSbxcBaseGame(
                roundHlrResultExtendSbxcBaseGame.getTotalWin(),
                roundHlrResultExtendSbxcBaseGame.getReSpinStrip(),
