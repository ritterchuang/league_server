...
import com.lx.gs.math.core.card.stackCtr.bjlStackCtr.BjlDealUtil;
...
import com.lx.gs.math.games.bjl_java.entity.PreGameAdjustResultBjlJava;
...
// 新百家樂發牌
public class ShuffleBjlJava extends AbstractShuffle implements IShuffle {
    private final LogicBjlJava logic; // 邏輯
    private final int DEAL_COUNT = 6; // 發牌數
    private final BjlDealUtil bjlDealUtil; // 百家樂發牌工具包

    public ShuffleBjlJava(AlgorithmTypeCtr algorithmTypeCtr, LogicBjlJava logic, ITableUtil tableUtil) {
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 4. 封裝
        return new PreGameAdjustResultBjlJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultBjlJava) preGameAdjustResult).getUnTakenCardListMap();
    ...
    public GameAdjustResult calculateGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 計算所有玩家輸贏結果
        Map<Integer, UidScore> uidScoreMap = this.logic.calculateUidScoreMap(((PreGameAdjustResultBjlJava) preGameAdjustResult).getUnTakenCardListMap());
