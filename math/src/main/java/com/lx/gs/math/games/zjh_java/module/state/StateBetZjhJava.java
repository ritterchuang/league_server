package com.lx.gs.math.games.zjh_java.module.state;
...
import com.lx.gs.math.games.zjh_java.entity.detail.DetailBet;
import com.lx.gs.math.games.zjh_java.entity.message.CtsBetZjhJava;
import com.lx.gs.math.games.zjh_java.entity.message.StcBetResultZjhJava;
import com.lx.gs.math.games.zjh_java.entity.message.StcBetZjhJava;
import com.lx.gs.math.games.zjh_java.entity.message.StcTurnZjhJava;
import com.lx.gs.math.games.zjh_java.module.gameAdjust.GameAdjustZjhJava;
import com.lx.gs.math.games.zjh_java.module.logic.LogicZjhJava;
import com.lx.gs.math.games.zjh_java.module.robotLogic.RobotLogicZjhJava;
...
// 下注狀態
public class StateBetZjhJava extends AbstractStateZjhJava {
    private static final String STATE_NAME = "bet";
    private static final Logger LOG = LoggerFactory.getLogger(StateBetZjhJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateBetZjhJava(TableZjhJava table, LogicZjhJava logic, RobotLogicZjhJava aiLogic, GameAdjustZjhJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 5. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), ConstZjhJava.TimeEnumZjhJava.BET.getCode(), ConstZjhJava.TimeEnumZjhJava.BET.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 玩家下注
    public void onMessageBet(int chairIndex, CtsBetZjhJava data){
    ...
    private void finishBetState(){
        ...
        // 4. 傳送下注結果
        StcBetResultZjhJava stcBetResult = new StcBetResultZjhJava(super.logic.getBetResultArray());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetResult);
        super.table.sendMessageToHumanPlayer(stcBetResult);

        // 5. 設置定時器
        super.table.getTableTimer().schedule(new TimerTaskTransState(this), ConstZjhJava.TimeEnumZjhJava.BET_RESULT.getMilliTimeSec());
    }
    ...
    private void sendCanBetRate() {
        StcTurnZjhJava stcTurn = new StcTurnZjhJava(
                ConstZjhJava.OperationEnumZjhJava.BET.getCode(),
                ConstZjhJava.TimeEnumZjhJava.BET.getTimeSec(),
                0,
    ...
    private void sendBetRate(int chairIndex, int rate) {
        StcBetZjhJava stcBet = new StcBetZjhJava(chairIndex, rate);
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateBetZjhJava state;

        public TimerTaskTimeOut(StateBetZjhJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransState extends TimerTask {
        ...
        public TimerTaskTransState(StateBetZjhJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotBet extends TimerTask {
        ...
        public TimerTaskRobotBet(StateBetZjhJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageBet(this.chairIndex, new CtsBetZjhJava(this.rate));
        }
    }
}