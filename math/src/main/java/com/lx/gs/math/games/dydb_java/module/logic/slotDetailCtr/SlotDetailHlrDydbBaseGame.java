package com.lx.gs.math.games.dydb_java.module.logic.slotDetailCtr;
...
import com.lx.gs.math.games.dydb_java.enity.result.slotDetail.SlotDetailDydbBaseGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者大運奪寶
public class SlotDetailHlrDydbBaseGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendDydbBaseGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrDydbBaseGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendDydbBaseGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailDydbBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 9. 回傳
        return new SlotDetailDydbBaseGame(spinCost, baseGameTotalWin, screenColumn_1, screenColumn_2, screenColumn_3, screenColumn_4, screenColumn_5);
    }
}