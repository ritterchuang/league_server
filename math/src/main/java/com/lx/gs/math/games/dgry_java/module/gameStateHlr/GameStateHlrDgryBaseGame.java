...
import com.lx.gs.math.games._develop_hj_java.enity.result.gameStateResult.GameStateHlrResultExtendDevelopHjBaseGame;
...
// 帝国榮耀 基本遊戲處理者
public class GameStateHlrDgryBaseGame extends AbstractGameStateHlr implements IGameStateHlr {
    private final LogicSlotCascade logic; // 牌桌邏輯
    private final GameStateConfigExtendDgryBaseGame gameStateConfigExtend; // 遊戲狀態額外設定
    private final RoundCascadeGameConfigExtendDgryBaseGame roundConfigExtend; // 消除局額外設定

    public GameStateHlrDgryBaseGame(int conditionId, int gameStateId, ITableUtilSlot tableUtil, ILogicSlot logic, GameStateConfig gameStateConfig) {
        super(conditionId, gameStateId, gameStateConfig, tableUtil);

        this.logic = (LogicSlotCascade) logic;
        this.gameStateConfigExtend = (GameStateConfigExtendDgryBaseGame) gameStateConfig.getGameStateConfigExtend();
        this.roundConfigExtend = (RoundCascadeGameConfigExtendDgryBaseGame) ((RoundConfigCascade)gameStateConfig.getRoundConfig()).getRoundCascadeGameConfigExtend();
    }
    ...
    public GameStateHlrResult calculateGameStateHlrResult(int gameStateResultIndex, ConstMathSlot.ReelRtpType reelRtpType, SpinRequest spinRequest, GameStateHlrResult beforeGameStateHlrResult) {
        ...
        for (int roundIndex = 0; roundIndex < roundCount; roundIndex++) {
            ...
            // 3. 計算特殊事件結果
            SpecialFeatureHlrResultCluster specialFeatureHlrResultCluster = this.logic.getSpecialFeatureHlrResultClusterCascade(
                    super.conditionId,
                    spinRequest,
                    ...
                    cascadeClusterHlrResult.getCascadeHlrResultList());
        ...
        return new GameStateHlrResult(
                ...
                new GameStateHlrResultExtendDevelopHjBaseGame(),
