...
import com.lx.gs.math.core.slot.cascadeClusterHlrMgr.module.cascadeClusterHlr.enity.config.CascadeClusterHlrConfigExtendCjwp;
import com.lx.gs.math.core.slot.cascadeClusterHlrMgr.module.cascadeClusterHlr.enity.result.CascadeClusterHlrResult;
import com.lx.gs.math.core.slot.cascadeClusterHlrMgr.module.cascadeClusterHlr.enity.result.CascadeClusterHlrResultExtend;
import com.lx.gs.math.core.slot.cascadeClusterHlrMgr.module.cascadeClusterHlr.enity.result.CascadeClusterHlrResultExtendCjwp;
...
// 消除集合處理者 超級王牌
public class CascadeClusterHlrCjwp implements ICascadeClusterHlr{
    ...
    private final CascadeClusterHlrConfigExtendCjwp configExtend; // 消除集合額外設定
    private final ICascadeHlr cascadeHlr; // 消除處理者
    private final ITableUtil tableUtil; // 牌桌工具包


    public CascadeClusterHlrCjwp(CascadeClusterHlrConfig config, AccumulateWinCtr accumulateWinCtr, ITableUtil tableUtil) {
        this.config = config;
        this.cascadeClusterType = config.getCascadeClusterType();
        this.configExtend = (CascadeClusterHlrConfigExtendCjwp) config.getHlrConfigExtend();
    ...
    private CascadeClusterHlrResultExtend calculateResultExtend(List<CascadeHlrResult> cascadeHlrResultList) {
        ...
        // 2. 取得消除前黃金位置
        List<List<Boolean>> beforeCascadeGoldenPosition = ((CascadeHlrResultExtendCjwp)cascadeHlrResultList.get(0).getCascadeHlrResultExtend()).getBeforeCascadeGoldenPosition();

        // 3. 複製設定檔
        CascadeClusterConfigExtendCjwp cascadeClusterConfigExtendCjwp = (CascadeClusterConfigExtendCjwp) this.configExtend.getConfigExtend();
        ...
        List<List<Integer>> result = ListUtil.copy2DimensionIntegerList((cascadeClusterConfigExtendCjwp).getDefaultGoldenPositionIdList());
        ...
        // 7. 回傳
        return new CascadeClusterHlrResultExtendCjwp(result);
    }
