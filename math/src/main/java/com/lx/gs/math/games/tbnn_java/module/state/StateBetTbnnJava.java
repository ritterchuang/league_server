package com.lx.gs.math.games.tbnn_java.module.state;

import com.lx.gs.math.core.common.lock.FinishLock;
import com.lx.gs.math.core.common.robotLogic.entity.RobotRateResultTonbi;
...
import com.lx.gs.math.games.tbnn_java.entity.detail.DetailBet;
import com.lx.gs.math.games.tbnn_java.entity.message.CtsBetTbnnJava;
import com.lx.gs.math.games.tbnn_java.entity.message.StcBetResultTbnnJava;
import com.lx.gs.math.games.tbnn_java.entity.message.StcBetTbnnJava;
import com.lx.gs.math.games.tbnn_java.entity.message.StcTurnTbnnJava;
import com.lx.gs.math.games.tbnn_java.module.gameAdjust.GameAdjustTbnnJava;
import com.lx.gs.math.games.tbnn_java.module.logic.LogicTbnnJava;
import com.lx.gs.math.games.tbnn_java.module.robotLogic.RobotLogicTbnnJava;
...
// 下注狀態
public class StateBetTbnnJava extends AbstractStateTbnnJava {
    private static final String STATE_NAME = "bet";
    private static final Logger LOG = LoggerFactory.getLogger(StateBetTbnnJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateBetTbnnJava(TableTbnnJava table, LogicTbnnJava logic, RobotLogicTbnnJava aiLogic, GameAdjustTbnnJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 5. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), ConstTbnnJava.TimeEnumTbnnJava.BET.getCode(), ConstTbnnJava.TimeEnumTbnnJava.BET.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 玩家下注
    public void onMessageBet(int chairIndex, CtsBetTbnnJava data){
    ...
    private void doRobotBet() {
        // 1. 計算機器人結果
        Map<Integer, RobotRateResultTonbi> robotResult = this.robotLogic.generateRobotBetResult();
    ...
    private void finishBetState(){
        ...
        // 4. 傳送下注結果
        StcBetResultTbnnJava stcBetResultTbnnJava = new StcBetResultTbnnJava(super.logic.getBetResultArray());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetResultTbnnJava);
        super.table.sendMessageToHumanPlayer(stcBetResultTbnnJava);

        // 5. 設置定時器
        super.table.getTableTimer().schedule(new TimerTaskTransState(this), ConstTbnnJava.TimeEnumTbnnJava.BET_RESULT.getMilliTimeSec());
    }
    ...
    private void sendCanBetRate() {
        StcTurnTbnnJava stcTurnTbnnJava = new StcTurnTbnnJava(
                ConstTbnnJava.OperationEnumTbnnJava.BET.getCode(),
                ConstTbnnJava.TimeEnumTbnnJava.BET.getTimeSec(),
                0,
                super.logic.getHumanCanBetRateArray());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcTurnTbnnJava);
        super.table.sendMessageToHumanPlayer(stcTurnTbnnJava);
    }

    // 傳送下注倍數
    private void sendBetRate(int chairIndex, int rate) {
        StcBetTbnnJava stcBetTbnnJava = new StcBetTbnnJava(chairIndex, rate);
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetTbnnJava);
        super.table.sendMessageToHumanPlayer(stcBetTbnnJava);
    }
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateBetTbnnJava state;

        public TimerTaskTimeOut(StateBetTbnnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransState extends TimerTask {
        ...
        public TimerTaskTransState(StateBetTbnnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotBet extends TimerTask {
        ...
        public TimerTaskRobotBet(StateBetTbnnJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageBet(this.chairIndex, new CtsBetTbnnJava(this.rate));
        }
    }
}