...
import com.lx.gs.math.games.lznn_java.entity.detail.DetailNoVie;
import com.lx.gs.math.games.lznn_java.entity.detail.DetailVieBanker;
import com.lx.gs.math.games.lznn_java.entity.detail.DetailVieResult;
import com.lx.gs.math.games.lznn_java.entity.message.CtsVieBankerLznnJava;
import com.lx.gs.math.games.lznn_java.entity.message.StcTurnLznnJava;
import com.lx.gs.math.games.lznn_java.entity.message.StcVieBankerLznnJava;
import com.lx.gs.math.games.lznn_java.entity.message.StcVieResultLznnJava;
...
// 搶莊狀態
public class StateVieBankerLznnJava extends AbstractStateLznnJava {
    private static final String STATE_NAME = "vieBanker";
    private static final Logger LOG = LoggerFactory.getLogger(StateVieBankerLznnJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateVieBankerLznnJava(TableLznnJava table, LogicLzNnJava logic, RobotLogicLznnJava aiLogic, GameAdjustLznnJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 4. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), ConstLznnJava.TimeEnumLznnJava.VIE_BANKER.getCode(), ConstLznnJava.TimeEnumLznnJava.VIE_BANKER.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 收到客戶端搶莊消息
    public void onMessageVieBanker(int chairIndex, CtsVieBankerLznnJava data){
    ...
    private void finishVieState(){
        ...
        if (super.logic.isShowVieBankAnimation()) {
            super.table.getTableTimer().schedule(new TimerTaskTransferState(this), ConstLznnJava.TimeEnumLznnJava.SHOW_BANKER.getMilliTimeSec());
        }else {
            super.iLeave();
        }
    }
    ...
    private void sendCanVieRate() {
        StcTurnLznnJava stcTurnLznnJava = new StcTurnLznnJava(
                ConstLznnJava.OperationEnumLznnJava.VIE.getCode(),
                ConstLznnJava.TimeEnumLznnJava.VIE_BANKER.getTimeSec(),
                0, // 原無效傳輸協議
    ...
    private void sendVieRate(int chairIndex, int rate) {
        StcVieBankerLznnJava stcVieBankerQznnJava = new StcVieBankerLznnJava(
                chairIndex,
                rate);
    ...
    private void sendVieResult() {
        StcVieResultLznnJava stcVieResultLznnJava = new StcVieResultLznnJava(
                super.logic.getBankerChairIndex(),
                super.logic.getBankerRate(),
                super.logic.getVieCandidateArrayMessage());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcVieResultLznnJava);
        super.table.sendMessageToHumanPlayer(stcVieResultLznnJava);
    }
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateVieBankerLznnJava state;

        public TimerTaskTimeOut(StateVieBankerLznnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransferState extends TimerTask {
        ...
        public TimerTaskTransferState(StateVieBankerLznnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotVieBank extends TimerTask {
        ...
        public TimerTaskRobotVieBank(StateVieBankerLznnJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageVieBanker(this.chairIndex, new CtsVieBankerLznnJava(this.rate));
        }
    }
    ...
    //* 詳細記錄相關 *//
    // 紀錄搶莊詳細紀錄
    private void addDetailVieBanker(int chairIndex, CtsVieBankerLznnJava data){
        if(data.getRate() <= 0){
