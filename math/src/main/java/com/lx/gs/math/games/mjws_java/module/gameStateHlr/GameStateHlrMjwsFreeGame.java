...
import com.lx.gs.math.games.mjws_java.enity.result.gameStateResult.GameStateHlrResultExtendMjwsBaseGame;
import com.lx.utils.MathUtil;

import java.util.ArrayList;

// 麻將無雙 基本遊戲處理者
public class GameStateHlrMjwsFreeGame extends AbstractGameStateHlr implements IGameStateHlr {
    private final LogicSlotCascade logic; // 牌桌邏輯
    private final GameStateConfigExtendMjwsFreeGame gameStateConfigExtend; // 遊戲狀態額外設定
    private final RoundCascadeGameConfigExtendMjwsFreeGame roundConfigExtend; // 消除局額外設定

    public GameStateHlrMjwsFreeGame(int conditionId, int gameStateId, ITableUtilSlot tableUtil, ILogicSlot logic, GameStateConfig gameStateConfig) {
        super(conditionId, gameStateId, gameStateConfig, tableUtil);

        this.logic = (LogicSlotCascade) logic;
        this.gameStateConfigExtend = (GameStateConfigExtendMjwsFreeGame) gameStateConfig.getGameStateConfigExtend();
        this.roundConfigExtend = (RoundCascadeGameConfigExtendMjwsFreeGame) ((RoundConfigCascade)gameStateConfig.getRoundConfig()).getRoundCascadeGameConfigExtend();
    }
    ...
    public GameStateHlrResult calculateGameStateHlrResult(int gameStateResultIndex, ConstMathSlot.ReelRtpType reelRtpType, SpinRequest spinRequest, GameStateHlrResult beforeGameStateHlrResult) {
        ...
        return new GameStateHlrResult(
                ...
                new GameStateHlrResultExtendMjwsBaseGame(),
