package com.lx.gs.math.games.sdzw_java.module.logic.slotDetailCtr;
...
import com.lx.gs.math.games.sdzw_java.enity.result.slotDetail.SlotDetailSdzwFreeGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 聖誕任務虎機免費遊戲
public class SlotDetailHlrSdzwFreeGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendSdzwFreeGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrSdzwFreeGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendSdzwFreeGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailSdzwFreeGame calculateFreeGameDetailPerRound(RoundHlrResult roundHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 10. 回傳
        return new SlotDetailSdzwFreeGame(
