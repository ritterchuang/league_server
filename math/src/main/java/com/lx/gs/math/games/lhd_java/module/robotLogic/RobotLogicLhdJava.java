package com.lx.gs.math.games.lhd_java.module.robotLogic;
...
import com.lx.gs.math.core.common.logic.logicBaiRen.AbstractLogicBaiRenLh;
...
// 新龍虎鬥機器人邏輯
public class RobotLogicLhdJava extends AbstractRobotLogic {
    ...
    private int calculateRandomAreaId(GamePlayer gamePlayer){
        // 1. 押龍則不能押虎
        if(this.calculateRobotAreaBetArray(gamePlayer.getChairIndex())[ConstLhdJava.BetAreaIdLhdJava.DRAGON.getCode()] > 0){
        ...
        // 2. 押虎則不能押龍
        if(this.calculateRobotAreaBetArray(gamePlayer.getChairIndex())[ConstLhdJava.BetAreaIdLhdJava.TIGER.getCode()] > 0){
            return super.tableUtil.getNotSupportSetRngDataRandomUtil().getRandomElement(
                    Arrays.stream(ConstLhdJava.BetAreaIdLhdJava.values()).filter(betAreaEnum ->
                                    betAreaEnum.getCode() != ConstLhdJava.BetAreaIdLhdJava.INVALID.getCode()
