package com.lx.gs.math.games.lmjjc_java.module.logic.slotDetailCtr;
...
import com.lx.gs.math.games.lmjjc_java.entity.result.slotDetail.SlotDetailLmjjcBaseGame;
import com.lx.gs.math.games.lmjjc_java.entity.result.slotDetail.SlotDetailLmjjcCascade;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 羅馬競技場 基本遊戲
public class SlotDetailHlrLmjjcBaseGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendLmjjcBaseGame extendConfig; // 虎機詳細記錄額外設定
    private final SlotDetailLmjjcUtil slotDetailLmjjcUtil; // 詳細記錄工具包

    public SlotDetailHlrLmjjcBaseGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil) {
        super(slotDetailHlrConfig, tableUtil);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendLmjjcBaseGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    ...
    // 計算一般遊戲詳細記錄
    private SlotDetailLmjjcBaseGame calculateDetailBaseGame(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 4. 回傳
        return new SlotDetailLmjjcBaseGame(spinCost, totalWin);
    }
