package com.lx.gs.math.games.pxky_java.module.gameStateHlr;
...
import com.lx.gs.math.core.slot.mathSlotConfigHlr.enity.sever.roundConfig.roundNormalGameConfig.RoundNormalGameConfigExtendPxkyBaseGame;
...
import com.lx.gs.math.games.pxky_java.enity.result.gameStateResult.GameStateHlrResultExtendPxkyBaseGame;
...
// 貔貅開運基礎遊戲處理者
public class GameStateHlrPxkyBaseGame extends AbstractGameStateHlr implements IGameStateHlr {
    private final LogicSlotNormal logic; // 牌桌邏輯
    private final GameStateConfigExtendPxkyBaseGame gameStateConfigExtend; // 遊戲狀態額外設定
    private final RoundNormalGameConfigExtendPxkyBaseGame roundConfigExtend; // 遊戲場次額外設定
    private final IFiniteAwardPoolHlr finiteAwardPoolHlr; // 有限獎項處理者

    public GameStateHlrPxkyBaseGame(int conditionId, int gameStateId, ITableUtilSlot tableUtil, ILogicSlot logic, IFiniteAwardPoolHlr finiteAwardPoolHlr, GameStateConfig gameStateConfig) {
        super(conditionId, gameStateId, gameStateConfig, tableUtil);
        this.logic = (LogicSlotNormal)logic;
        this.gameStateConfigExtend = (GameStateConfigExtendPxkyBaseGame) gameStateConfig.getGameStateConfigExtend();
        this.roundConfigExtend = (RoundNormalGameConfigExtendPxkyBaseGame) ((RoundConfigNormal)gameStateConfig.getRoundConfig()).getRoundNormalGameConfigExtend();
    ...
    private RoundHlrResultExtend calculateRoundHlrResultExtend(int conditionId, List<DampScreenBox> dampScreenBoxList) {
       ...
       List<Integer> reSpinReelStrip = this.getReSpinReelStrip(reSpinScreenGtrResult.getReelCtrResult().getReelId());
        ...
        // 5. 回傳
        return new RoundHlrResultExtendPxkyBaseGame(
