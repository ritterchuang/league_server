...
import com.lx.gs.math.core.fish.hitCtrMgr.module.hitCtr.hitCtrUtil.module.HitCtrUtilRedEnvelope;

// 紅包目標打擊計算器
public class HitCtrRedEnvelope extends AbstractHitCtr implements IHitCtr {
    private final ConstMathFish.HitType hitType; // 打擊類型
    private final HitTypeConfigExtendRedEnvelope config; // 客製打擊計算器設定
    private final HitCtrUtilRedEnvelope hitCtrUtil; // 打擊計算工具包

    public HitCtrRedEnvelope(HitCtrConfig hitCtrConfig, GamePlayerHlr gamePlayerHlr, PoolCtr poolCtr, ITableUtil tableUtil) {
        super(hitCtrConfig, gamePlayerHlr, poolCtr, tableUtil);
        this.hitType = hitCtrConfig.getHitType();
        this.config = (HitTypeConfigExtendRedEnvelope) hitCtrConfig.getHitTypeConfigExtend();
        this.hitCtrUtil = new HitCtrUtilRedEnvelope(this.config, tableUtil);
    }
