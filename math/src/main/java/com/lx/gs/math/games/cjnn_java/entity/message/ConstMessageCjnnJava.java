...
// 新紅黑大戰消息定義
public class ConstMessageCjnnJava {
    // 物件列舉對應表
    public static final Map<String, IMessageEnum> MessageObjectEnumMap = new HashMap<>(){{

        //* STC相關 *//
        put(StcCutReturnCjnnJava.class.getCanonicalName(), MessageEnum.STC_CUT_RETURN); // 回復資訊
        put(StcUpdateUserCjnnJava.class.getCanonicalName(), MessageEnum.STC_UPDATE_USER); // 更新玩家資訊 TODO 使用 UpdateUserGame
        put(StcChartCjnnJava.class.getCanonicalName(), MessageEnum.STC_CHART); // 路線圖
        put(StcGameResultCjnnJava.class.getCanonicalName(), MessageEnum.STC_GAME_RESULT); // 遊戲結果
    }};
