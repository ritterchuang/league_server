...
// 消除畫面結果包裝者
public class CascadeScreenResultPgrDgry extends CascadeScreenResultPgr{

    public CascadeScreenResultPgrDgry(CascadeScreenResultPgrConfig config, ITableUtil tableUtil) {
    ...
    public ScreenResult calculateCascadeScreenResult(ScreenResult lastScreenResult, CascadeResultExtend lastCascadeResultExtend, ScreenGtrResult currentScreenGtrResult) {
        // 1. 轉型上次消除結果
        CascadeResultExtendDgry cascadeResultExtendDgry = (CascadeResultExtendDgry) lastCascadeResultExtend;
        ...
        // 4. 計算此次消除前畫面
        List<List<SymbolBox>> screenSymbolBoxWithDamp = this.calculateScreenSymbolBoxWithDamp(cascadeResultExtendDgry.getScreenSymbolBoxAfterEliminate(), addSymbolResult);
