package com.lx.gs.math.games.xzcq_java.module.logic.slotDetailCtr;
...
import com.lx.gs.math.games.xzcq_java.entity.result.roundResult.RoundHlrResultExtendXzcqBaseGame;
import com.lx.gs.math.games.xzcq_java.entity.result.slotDetail.SlotDetailXzcqBaseGame;
import com.lx.gs.math.games.xzcq_java.entity.result.slotDetail.SlotDetailXzcqReSpin;
...
// 虎機詳細記錄計算者小豬傳奇
public class SlotDetailHlrXzcq extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendXzcq extendConfig; // 虎機詳細記錄額外設定
    private final int emptySymbolId; // 空白標誌識別碼

    public SlotDetailHlrXzcq(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendXzcq) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    ...
    public List<SlotDetail> calculateSlotDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        ...
        // 3. 計算重轉遊戲
        Optional<SlotDetailXzcqReSpin> slotDetailXzcqReSpin = this.calculateReSpinDetail(gameStateHlrResult);
        if (slotDetailXzcqReSpin.isPresent()) {
            result.add(slotDetailXzcqReSpin.get());
        }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailXzcqBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 7. 回傳
        return new SlotDetailXzcqBaseGame(spinCost, screenColumn_1, screenColumn_2, screenColumn_3, baseGameTotalWin);
    }


    // 計算重轉遊戲詳細記錄
    private Optional<SlotDetailXzcqReSpin> calculateReSpinDetail(GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        RoundHlrResultNormal roundHlrResultNormal = ((RoundHlrResultNormal) gameStateHlrResult.getRoundHlrResultList().get(0));
        ...
        // 7. 回傳
        return Optional.of(new SlotDetailXzcqReSpin(screenColumn_1, screenColumn_2, screenColumn_3, reSpinTotalWin));
    }
    ...
    private String calculateReSpinSymbol(RoundHlrResultExtend roundHlrResultExtend) {
        ...
        // 2. 轉型局處理者額外結果
        RoundHlrResultExtendXzcqBaseGame roundHlrResultExtendXzcqBaseGame = (RoundHlrResultExtendXzcqBaseGame) roundHlrResultExtend;

        // 3. 依照設定檔取得標誌
        int reSpinSymbolId = this.calculateReSpinSymbolId(roundHlrResultExtendXzcqBaseGame);
    ...
    // 計算重轉標誌
    private int calculateReSpinSymbolId(RoundHlrResultExtendXzcqBaseGame roundHlrResultExtendXzcqBaseGame) {
        // 1. 取得停輪位置
        int reelStopIndex = ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendXzcqBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().get(
                ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendXzcqBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().size() - 1);

        // 2. 回傳停輪標誌
        return roundHlrResultExtendXzcqBaseGame.getReSpinStrip().get(reelStopIndex);
    }
}
