package com.lx.gs.math.games.qznn_k4z_java.module.gameAdjust;
...
import com.lx.gs.math.games.qznn_k4z_java.entity.PreGameAdjustResultQznnK4ZJava;
...
// 新看四張搶莊牛牛發牌
public class ShuffleQznnK4zJava extends AbstractShuffle implements IShuffle {
    private final LogicQznnK4zJava logic; // 邏輯
    private final int DEAL_COUNT_PER_PLAYER = 1; // 每人發牌數

    public ShuffleQznnK4zJava(AlgorithmTypeCtr algorithmTypeCtr, LogicQznnK4zJava logicQZNN, ITableUtil tableUtil) {
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 5. 封裝
        return new PreGameAdjustResultQznnK4ZJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultQznnK4ZJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
    ...
    private Map<Integer, List<ICard>> calculateMergerPlayerCardList(PreGameAdjustResult preGameAdjustResult){
        ...
        // 2. 後一張手牌
        Map<Integer, List<ICard>> playerCardListLastTwo = ((PreGameAdjustResultQznnK4ZJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
