...
import com.lx.gs.math.core.fish.hitCtrMgr.entity.server.hitResultExtend.HitResultExtendWheel;
import com.lx.gs.math.core.fish.hitCtrMgr.module.hitCtr.awardBulletGtr.module.IAwardBulletGtr;
import com.lx.gs.math.core.fish.hitCtrMgr.module.hitCtr.robotResultWpr.robotResultCtr.enity.config.RobotResultCtrConfigExtendWheel;
...
// 機器人結果計算者輪盤
public class RobotResultCtrWheel extends AbstractRobotResultCtr implements IRobotResultCtr {
    private final ConstMathFish.HitType hitType; // 機器人結果打擊類型
    private final RobotResultCtrConfigExtendWheel config; // 機器人結果打擊設定

    public RobotResultCtrWheel(ConstMathFish.HitType hitType, RobotResultCtrConfigExtendWheel robotResultCtrConfigExtendWheel, double robotRtp) {
        super(robotRtp);
        this.hitType = hitType;
        this.config = robotResultCtrConfigExtendWheel;
    }
    ...
    public List<RobotBasicOddsInfo> getRobotBasicOddsInfoList() {
        ...
        // 2. 計算倍數權重和
        int totalWeight = Arrays.stream(this.config.getShowOddsWeightArray()).sum();

        // 3. 添加倍數資訊
        for (int oddsIndex = 0; oddsIndex < this.config.getShowOddsArray().length; oddsIndex++) {
            result.add(new RobotBasicOddsInfo(this.config.getShowOddsArray()[oddsIndex], MathUtil.divide(this.config.getShowOddsWeightArray()[oddsIndex], totalWeight)));
        }
