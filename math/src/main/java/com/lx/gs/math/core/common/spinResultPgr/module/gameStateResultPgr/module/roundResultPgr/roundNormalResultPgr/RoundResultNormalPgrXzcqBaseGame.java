...
import com.lx.gs.math.games.xzcq_java.entity.result.roundResult.RoundResultExtendXzcqBaseGame;

// 客製局結果包裝者一般 小豬傳奇基礎遊戲
public class RoundResultNormalPgrXzcqBaseGame extends AbstractRoundResultNormalPgr {
    private final int reSpinColumnIndex = 2;

    public RoundResultNormalPgrXzcqBaseGame(ClientRoundResultPgrConfig config, ITableUtil tableUtil) {
    ...
    public RoundResultExtend packageRoundResultExtend(RoundHlrResultNormal roundHlrResultNormal) {
        ...
        // 2. 轉型777運算結果
        RoundHlrResultExtendXzcqBaseGame roundHlrResultExtendXzcqBaseGame = (RoundHlrResultExtendXzcqBaseGame) (roundHlrResultNormal.getRoundHlrResultExtend());

        // 3. 計算客端重轉畫面
        ScreenResult clientReSpinScreenResult = this.clientScreenPgr.calculateScreenResult(roundHlrResultExtendXzcqBaseGame.getScreenGtrResult());

        // 4. 計算客端重轉算分
        GameResult clientGameResult = super.packageGameResult(clientReSpinScreenResult, roundHlrResultExtendXzcqBaseGame.getReSpinGameCtrResult());

        // 5. 回傳
        return new RoundResultExtendXzcqBaseGame(
                roundHlrResultExtendXzcqBaseGame.getTotalWin(),
                roundHlrResultExtendXzcqBaseGame.getReSpinStrip(),
