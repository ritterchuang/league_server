...
import com.lx.gs.math.games.ebg_java.entity.detail.DetailNoVie;
import com.lx.gs.math.games.ebg_java.entity.detail.DetailVieBanker;
import com.lx.gs.math.games.ebg_java.entity.detail.DetailVieResult;
import com.lx.gs.math.games.ebg_java.entity.message.CtsVieBankerEbgJava;
import com.lx.gs.math.games.ebg_java.entity.message.StcTurnEbgJava;
import com.lx.gs.math.games.ebg_java.entity.message.StcVieBankerEbgJava;
import com.lx.gs.math.games.ebg_java.entity.message.StcVieResultEbgJava;
...
// 搶莊狀態 TODO 邏輯修改
public class StateVieBankerEbgJava extends AbstractStateEbgJava {
    private static final String STATE_NAME = "vieBanker";
    private static final Logger LOG = LoggerFactory.getLogger(StateVieBankerEbgJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateVieBankerEbgJava(TableEbgJava table, LogicEbgJava logic, RobotLogicEbgJava aiLogic, GameAdjustEbgJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 4. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), ConstEbgJava.TimeEnumEbgJava.VIE_BANKER.getCode(), ConstEbgJava.TimeEnumEbgJava.VIE_BANKER.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 收到客戶端搶莊消息
    public void onMessageVieBanker(int chairIndex, CtsVieBankerEbgJava data){
    ...
    private void finishVieState(){
        ...
        if (super.logic.isShowVieBankAnimation()) {
            long showBankerTime = (long) (1.5 * 1000) + ConstEbgJava.TimeEnumEbgJava.SHOW_BANKER.getMilliTimeSec();
    ...
    private void sendCanVieRate() {
        StcTurnEbgJava stcTurn = new StcTurnEbgJava(
                ConstEbgJava.OperationEnumEbgJava.VIE.getCode(),
                ConstEbgJava.TimeEnumEbgJava.VIE_BANKER.getTimeSec(),
    ...
    private void sendVieRate(int chairIndex, int rate) {
        StcVieBankerEbgJava stcVieBanker = new StcVieBankerEbgJava(
                chairIndex,
                rate);
    ...
    private void sendVieResult() {
        StcVieResultEbgJava stcVieResult = new StcVieResultEbgJava(
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateVieBankerEbgJava state;

        public TimerTaskTimeOut(StateVieBankerEbgJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransferState extends TimerTask {
        ...
        public TimerTaskTransferState(StateVieBankerEbgJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotVieBank extends TimerTask {
        ...
        public TimerTaskRobotVieBank(StateVieBankerEbgJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageVieBanker(this.chairIndex, new CtsVieBankerEbgJava(this.rate));
        }
    }
    ...
    //* 詳細記錄相關 *//
    // 紀錄搶莊詳細紀錄
    private void addDetailVieBanker(int chairIndex, CtsVieBankerEbgJava data){
        if(data.getRate() <= 0){
