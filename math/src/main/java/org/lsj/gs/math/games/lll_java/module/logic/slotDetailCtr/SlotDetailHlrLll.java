package org.lsj.gs.math.games.lll_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games.lll_java.entity.result.roundResult.RoundHlrResultExtendLllBaseGame;
import org.lsj.gs.math.games.lll_java.entity.result.slotDetail.SlotDetailLllBaseGame;
import org.lsj.gs.math.games.lll_java.entity.result.slotDetail.SlotDetailLllReSpin;
...
// 虎機詳細記錄計算者龍龍龍
public class SlotDetailHlrLll extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendLll extendConfig; // 虎機詳細記錄額外設定
    private final int emptySymbolId; // 空白標誌識別碼

    public SlotDetailHlrLll(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendLll) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    ...
    public List<SlotDetail> calculateSlotDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        ...
        // 3. 計算重轉遊戲
        Optional<SlotDetailLllReSpin> slotDetailLllReSpin = this.calculateReSpinDetail(gameStateHlrResult);
        if (slotDetailLllReSpin.isPresent()) {
            result.add(slotDetailLllReSpin.get());
        }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailLllBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 7. 回傳
        return new SlotDetailLllBaseGame(spinCost, screenColumn_1, screenColumn_2, screenColumn_3, baseGameTotalWin);
    }


    // 計算重轉遊戲詳細記錄
    private Optional<SlotDetailLllReSpin> calculateReSpinDetail(GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        RoundHlrResultNormal roundHlrResultNormal = ((RoundHlrResultNormal) gameStateHlrResult.getRoundHlrResultList().get(0));
        ...
        // 7. 回傳
        return Optional.of(new SlotDetailLllReSpin(screenColumn_1, screenColumn_2, screenColumn_3, reSpinTotalWin));
    }
    ...
    private String calculateReSpinSymbol(RoundHlrResultExtend roundHlrResultExtend) {
        ...
        // 2. 轉型局處理者額外結果
        RoundHlrResultExtendLllBaseGame roundHlrResultExtendLllBaseGame = (RoundHlrResultExtendLllBaseGame) roundHlrResultExtend;

        // 3. 依照設定檔取得標誌
        int reSpinSymbolId = this.calculateReSpinSymbolId(roundHlrResultExtendLllBaseGame);
    ...
    // 計算重轉標誌
    private int calculateReSpinSymbolId(RoundHlrResultExtendLllBaseGame roundHlrResultExtendLllBaseGame) {
        // 1. 取得停輪位置
        int reelStopIndex = ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendLllBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().get(
                ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendLllBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().size() - 1);

        // 2. 回傳停輪標誌
        return roundHlrResultExtendLllBaseGame.getReSpinStrip().get(reelStopIndex);
    }
}
