...
import org.lsj.gs.math.games.pxky_java.enity.result.roundResult.RoundHlrResultExtendPxkyBaseGame;
import org.lsj.gs.math.games.pxky_java.enity.result.roundResult.RoundResultExtendPxkyBaseGame;

// 客製局結果包裝者一般 豼貅開運基礎遊戲
public class RoundResultNormalPgrPxkyBaseGame extends AbstractRoundResultNormalPgr {
    private final int reSpinColumnIndex = 2;

    public RoundResultNormalPgrPxkyBaseGame(ClientRoundResultPgrConfig config, ITableUtil tableUtil) {
    ...
    public RoundResultExtend packageRoundResultExtend(RoundHlrResultNormal roundHlrResultNormal) {
        ...
        // 2. 轉型貔貅額外運算結果
        RoundHlrResultExtendPxkyBaseGame roundHlrResultExtendPxkyBaseGame = (RoundHlrResultExtendPxkyBaseGame)(roundHlrResultNormal.getRoundHlrResultExtend());

        // 3. 計算客端重轉畫面
        ScreenResult clientReSpinScreenResult = this.clientScreenPgr.calculateScreenResult(roundHlrResultExtendPxkyBaseGame.getScreenGtrResult());

        // 4. 計算客端重轉算分
        GameResult clientGameResult = super.packageGameResult(clientReSpinScreenResult, roundHlrResultExtendPxkyBaseGame.getReSpinGameCtrResult());

        // 5. 回傳
        return new RoundResultExtendPxkyBaseGame(
                roundHlrResultExtendPxkyBaseGame.getTotalWin(),
                roundHlrResultExtendPxkyBaseGame.getReSpinStrip(),
