package org.lsj.gs.math.games.cjnn_java;
...
import org.lsj.gs.math.games.cjnn_java.entity.message.StcChartCjnnJava;
import org.lsj.gs.math.games.cjnn_java.entity.message.StcCutReturnCjnnJava;
import org.lsj.gs.math.games.cjnn_java.module.chartCtr.ChartCtrCjnnJava;
...
import org.lsj.gs.math.games.cjnn_java.module.state.StateGameBeginCjnnJava;
import org.lsj.gs.math.games.cjnn_java.module.state.StateGameEndCjnnJava;
import org.lsj.gs.math.games.cjnn_java.module.state.StateWaitBeginCjnnJava;
...
// 新超級牛牛遊戲桌
public class TableCjnnJava extends AbstractTableMessageBaiRen {
    private static final Logger LOG = LoggerFactory.getLogger(TableCjnnJava.class);

    private final ChartCtrCjnnJava chartCtr; // 路圖計算器
    private final LogicCjnnJava logic; // 遊戲邏輯
    private final RobotLogicCjnnJava robotLogic; // 機器人遊戲邏輯
    private final GameAdjustCjnnJava gameAdjust; // 遊戲調控

    public TableCjnnJava(int tableId,
        ...
        super(  tableFieldConfig,
                ...
                user,
                tableUtil);
        this.chartCtr = new ChartCtrCjnnJava(tableUtil);
        this.logic = new LogicCjnnJava(this,
                tableFieldConfig,
                tableGameConfig,
                ...
                new GameBetLogResultCtrBaiRen(
                        ...
                        tableUtil),
                tableUtil);
        this.robotLogic = new RobotLogicCjnnJava(
                super.getGamePlayerHlr(),
                super.getPoolCtr(),
                tableUtil,
                this.logic);
        this.gameAdjust = new GameAdjustCjnnJava(
    ...
    public void sendChart(){
        if(super.getGamePlayerHlr().isHumanEnterTable()) {
            StcChartCjnnJava stcChart = new StcChartCjnnJava(this.chartCtr.getChartArray());
    ...
    // 斷線重連資訊
    private StcCutReturnCjnnJava calculateCutReturn(double leftTimeSec){
        // 1. 計算當前狀態
        ...
        return new StcCutReturnCjnnJava(stage, leftTime, areas, luckyStarAreas, types, cards, lifts, result);
    }
    ...
    public void registerState(){
        // 1. 註冊狀態機
        super.getStateMgr().registerState(new StateWaitBeginCjnnJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.WAIT_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameBeginCjnnJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameEndCjnnJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_END.getCode()));
    ...
    private void receivedGameCommandBet(ReceiveGameCommand receiveGameCommand){
        ...
        // 2. 請求
        try {
            ((StateGameBeginCjnnJava)super.stateMgr.getStateMap().get(BaiRenGameState.GAME_BEGIN.getCode()))
