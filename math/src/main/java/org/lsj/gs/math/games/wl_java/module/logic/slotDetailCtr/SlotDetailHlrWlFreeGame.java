package org.lsj.gs.math.games.wl_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games.wl_java.enity.result.slotDetail.SlotDetailWlFreeGame;
...
// 虎機詳細記錄計算者 旺來免費遊戲
public class SlotDetailHlrWlFreeGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendWlFreeGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrWlFreeGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendWlFreeGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailWlFreeGame calculateFreeGameDetailPerRound(RoundHlrResult roundHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 13. 回傳
        return new SlotDetailWlFreeGame(
    ...
    private String calculateMultiplierListPerColumn(RoundHlrResultNormal roundHlrResultNormal, int columnIndex) {
        ...
        // 3. 取出額外結果
        RoundHlrResultExtendWlFreeGame roundHlrResultExtendWlFreeGame = (RoundHlrResultExtendWlFreeGame) roundHlrResultNormal.getRoundHlrResultExtend();
        List<Integer> multiplierListPerColumn = roundHlrResultExtendWlFreeGame.getMultiplierMatrix().get(columnIndex);
