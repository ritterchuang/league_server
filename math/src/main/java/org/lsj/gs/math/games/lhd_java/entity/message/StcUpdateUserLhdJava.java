...
// 更新玩家訊息 TODO 沒被呼叫過，待確認刪除
public class StcUpdateUserLhdJava extends AbstractStcMessageLhdJava {
    @JsonIgnore public int uid; // 玩家識別碼
    @JsonIgnore public int chair; // 位置
    @JsonIgnore public int role; // 角色
    @JsonIgnore public int state; // 狀態

    public StcUpdateUserLhdJava(int uid, int chair, int role, int state) {
        super(StcUpdateUserLhdJava.class.getCanonicalName());
        this.uid = uid;
        this.chair = chair;
        this.role = role;
        this.state = state;
