...
import org.lsj.gs.math.games.wl_java.enity.result.gameStateResult.GameStateHlrResultExtendWlBaseGame;
import org.lsj.utils.MathUtil;

import java.util.ArrayList;

// 旺來基礎遊戲處理者
public class GameStateHlrWlBaseGame extends AbstractGameStateHlr implements IGameStateHlr {
    private final LogicSlotNormal logic; // 牌桌邏輯
    private final GameStateConfigExtendWlBaseGame gameStateConfigExtend; // 遊戲狀態額外設定
    private final RoundNormalGameConfigExtendWlBaseGame roundConfigExtend; // 遊戲場次額外設定

    public GameStateHlrWlBaseGame(int conditionId, int gameStateId, ITableUtilSlot tableUtil, ILogicSlot logic, GameStateConfig gameStateConfig) {
        super(conditionId, gameStateId, gameStateConfig, tableUtil);
        this.logic = (LogicSlotNormal) logic;
        this.gameStateConfigExtend = (GameStateConfigExtendWlBaseGame) gameStateConfig.getGameStateConfigExtend();
        this.roundConfigExtend = (RoundNormalGameConfigExtendWlBaseGame) ((RoundConfigNormal)gameStateConfig.getRoundConfig()).getRoundNormalGameConfigExtend();
    }
    ...
    public GameStateHlrResult calculateGameStateHlrResult(int gameStateResultIndex, ConstMathSlot.ReelRtpType reelRtpType, SpinRequest spinRequest, GameStateHlrResult beforeGameStateHlrResult) {
        ...
        return new GameStateHlrResult(
                ...
                new GameStateHlrResultExtendWlBaseGame(),
