...
import org.lsj.gs.math.games.sg_java.entity.PreGameAdjustResultSgJava;
...
// 新三公發牌
public class ShuffleSgJava extends AbstractShuffle implements IShuffle {
    private final LogicSgJava logic; // 邏輯
    private final int DEAL_COUNT_PER_PLAYER = 3; // 每人發牌數

    public ShuffleSgJava(AlgorithmTypeCtr algorithmTypeCtr, LogicSgJava logicSgJava, ITableUtil tableUtil) {
        super(algorithmTypeCtr, tableUtil);
        this.logic = logicSgJava;
    }
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 5. 封裝
        return new PreGameAdjustResultSgJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultSgJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
    ...
    public GameAdjustResult calculateGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 整合牌
        Map<Integer, List<ICard>> playerCardList = ((PreGameAdjustResultSgJava)preGameAdjustResult).getAllPlayerUnTakenCardListMap();
