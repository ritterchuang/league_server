...
import org.lsj.gs.math.games.zcjz_java.entity.result.roundResult.RoundResultExtendZcjzBaseGame;

// 客製局結果包裝者一般 招財金豬基礎遊戲
public class RoundResultNormalPgrZcjzBaseGame extends AbstractRoundResultNormalPgr {
    private final int reSpinColumnIndex = 2;

    public RoundResultNormalPgrZcjzBaseGame(ClientRoundResultPgrConfig config, ITableUtil tableUtil) {
    ...
    public RoundResultExtend packageRoundResultExtend(RoundHlrResultNormal roundHlrResultNormal) {
        ...
        // 2. 轉型招財金豬運算結果
        RoundHlrResultExtendZcjzBaseGame roundHlrResultExtendZcjzBaseGame = (RoundHlrResultExtendZcjzBaseGame) (roundHlrResultNormal.getRoundHlrResultExtend());

        // 3. 計算客端重轉畫面
        ScreenResult clientReSpinScreenResult = this.clientScreenPgr.calculateScreenResult(roundHlrResultExtendZcjzBaseGame.getScreenGtrResult());

        // 4. 計算客端重轉算分
        GameResult clientGameResult = super.packageGameResult(clientReSpinScreenResult, roundHlrResultExtendZcjzBaseGame.getReSpinGameCtrResult());

        // 5. 回傳
        return new RoundResultExtendZcjzBaseGame(
                roundHlrResultExtendZcjzBaseGame.getTotalWin(),
                roundHlrResultExtendZcjzBaseGame.getReSpinStrip(),
