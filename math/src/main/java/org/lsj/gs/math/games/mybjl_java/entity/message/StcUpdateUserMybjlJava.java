...
// 更新玩家訊息
public class StcUpdateUserMybjlJava extends AbstractStcMessageMybjlJava {
    @JsonIgnore public int uid; // 玩家識別碼
    @JsonIgnore public int chair; // 位置
    @JsonIgnore public int role; // 角色
    @JsonIgnore public int state; // 狀態

    public StcUpdateUserMybjlJava(int uid, int chair, int role, int state) {
        super(StcUpdateUserMybjlJava.class.getCanonicalName());
        this.uid = uid;
        this.chair = chair;
        this.role = role;
        this.state = state;
