package org.lsj.gs.math.games.zcjz_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games.zcjz_java.entity.result.roundResult.RoundHlrResultExtendZcjzBaseGame;
import org.lsj.gs.math.games.zcjz_java.entity.result.slotDetail.SlotDetailZcjzBaseGame;
import org.lsj.gs.math.games.zcjz_java.entity.result.slotDetail.SlotDetailZcjzReSpin;
...
// 虎機詳細記錄計算者招財金豬
public class SlotDetailHlrZcjz extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendZcjz extendConfig; // 虎機詳細記錄額外設定
    private final int emptySymbolId; // 空白標誌識別碼

    public SlotDetailHlrZcjz(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendZcjz) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    ...
    public List<SlotDetail> calculateSlotDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        ...
        // 3. 計算重轉遊戲
        Optional<SlotDetailZcjzReSpin> slotDetailZcjzReSpin = this.calculateReSpinDetail(gameStateHlrResult);
        if (slotDetailZcjzReSpin.isPresent()) {
            result.add(slotDetailZcjzReSpin.get());
        }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailZcjzBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 7. 回傳
        return new SlotDetailZcjzBaseGame(spinCost, screenColumn_1, screenColumn_2, screenColumn_3, baseGameTotalWin);
    }


    // 計算重轉遊戲詳細記錄
    private Optional<SlotDetailZcjzReSpin> calculateReSpinDetail(GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        RoundHlrResultNormal roundHlrResultNormal = ((RoundHlrResultNormal) gameStateHlrResult.getRoundHlrResultList().get(0));
        ...
        // 7. 回傳
        return Optional.of(new SlotDetailZcjzReSpin(screenColumn_1, screenColumn_2, screenColumn_3, reSpinTotalWin));
    }
    ...
    private String calculateReSpinSymbol(RoundHlrResultExtend roundHlrResultExtend) {
        ...
        // 2. 轉型局處理者額外結果
        RoundHlrResultExtendZcjzBaseGame roundHlrResultExtendZcjzBaseGame = (RoundHlrResultExtendZcjzBaseGame) roundHlrResultExtend;

        // 3. 依照設定檔取得標誌
        int reSpinSymbolId = this.calculateReSpinSymbolId(roundHlrResultExtendZcjzBaseGame);
    ...
    // 計算重轉標誌
    private int calculateReSpinSymbolId(RoundHlrResultExtendZcjzBaseGame roundHlrResultExtendZcjzBaseGame) {
        // 1. 取得停輪位置
        int reelStopIndex = ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendZcjzBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().get(
                ((ReelStopResultExtendStopByDependentReelIndex)roundHlrResultExtendZcjzBaseGame.getScreenGtrResult().getReelCtrResult().getReelStopResultExtend()).getReelStopIndexList().size() - 1);

        // 2. 回傳停輪標誌
        return roundHlrResultExtendZcjzBaseGame.getReSpinStrip().get(reelStopIndex);
    }
}
