...
import org.lsj.gs.math.games.xjtb_java.entity.result.slotDetail.SlotDetailXjtbBaseGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者仙境探寶
public class SlotDetailHlrXjtbBaseGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendXjtbBaseGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrXjtbBaseGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendXjtbBaseGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailXjtbBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 9. 回傳
        return new SlotDetailXjtbBaseGame(spinCost, baseGameTotalWin, screenColumn_1, screenColumn_2, screenColumn_3, screenColumn_4, screenColumn_5);
    }
}