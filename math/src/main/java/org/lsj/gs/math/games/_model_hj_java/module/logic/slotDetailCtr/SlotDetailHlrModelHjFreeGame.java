...
import org.lsj.gs.math.games._model_hj_java.enity.result.slotDetail.SlotDetailModelHjFreeGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 模板虎機免費遊戲
public class SlotDetailHlrModelHjFreeGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendModelHjFreeGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrModelHjFreeGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendModelHjFreeGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailModelHjFreeGame calculateFreeGameDetailPerRound(RoundHlrResult roundHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 10. 回傳
        return new SlotDetailModelHjFreeGame(
