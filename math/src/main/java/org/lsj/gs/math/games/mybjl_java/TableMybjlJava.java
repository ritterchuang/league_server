...
import org.lsj.gs.math.games.mybjl_java.entity.message.StcChartMybjlJava;
import org.lsj.gs.math.games.mybjl_java.entity.message.StcCutReturnMybjlJava;
import org.lsj.gs.math.games.mybjl_java.module.chartCtr.ChartCtrMybjlJava;
...
import org.lsj.gs.math.games.mybjl_java.module.state.StateGameBeginMybjlJava;
import org.lsj.gs.math.games.mybjl_java.module.state.StateGameEndMybjlJava;
import org.lsj.gs.math.games.mybjl_java.module.state.StateWaitBeginMybjlJava;
...
// 新免傭百家樂遊戲桌
public class TableMybjlJava extends AbstractTableMessageBaiRen {
    private static final Logger LOG = LoggerFactory.getLogger(TableMybjlJava.class);

    private final ChartCtrMybjlJava chartCtr; // 路圖計算器
    private final LogicMybjlJava logic; // 遊戲邏輯
    private final RobotLogicMybjlJava robotLogic; // 機器人遊戲邏輯
    private final GameAdjustMybjlJava gameAdjust; // 遊戲調控

    public TableMybjlJava(int tableId,
        ...
        super(  tableFieldConfig,
                ...
                user,
                tableUtil);
        this.chartCtr = new ChartCtrMybjlJava(tableUtil);
        this.logic = new LogicMybjlJava(this,
                tableFieldConfig,
                tableGameConfig,
                ...
                new GameBetLogResultCtrBaiRen(
                        ...
                        tableUtil),
                tableUtil);
        this.robotLogic = new RobotLogicMybjlJava(
                super.getGamePlayerHlr(),
                super.getPoolCtr(),
                tableUtil,
                this.logic);
        this.gameAdjust = new GameAdjustMybjlJava(
    ...
    public void sendChart(){
        if(super.getGamePlayerHlr().isHumanEnterTable()) {
            StcChartMybjlJava stcChart = new StcChartMybjlJava(this.chartCtr.getChartArray());
    ...
    // 斷線重連資訊
    private StcCutReturnMybjlJava calculateCutReturn(double leftTimeSec){
        // 1. 計算當前狀態
        ...
        return new StcCutReturnMybjlJava(stage, leftTime, areas, luckyStarAreas, playCards, bankCards, compareScore);
    }
    ...
    public void registerState(){
        // 1. 註冊狀態機
        super.getStateMgr().registerState(new StateWaitBeginMybjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.WAIT_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameBeginMybjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_BEGIN.getCode()));
        super.getStateMgr().registerState(new StateGameEndMybjlJava(this, this.logic, this.robotLogic, this.gameAdjust, BaiRenGameState.GAME_END.getCode()));
    ...
    private void receivedGameCommandBet(ReceiveGameCommand receiveGameCommand){
        ...
        // 2. 請求
        try {
            ((StateGameBeginMybjlJava)super.stateMgr.getStateMap().get(BaiRenGameState.GAME_BEGIN.getCode()))
    ...
    //* 牌桌資訊 *//
    public LogicMybjlJava getLogic(){
        return this.logic;
    }
