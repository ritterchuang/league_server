package org.lsj.gs.math.games.olldbz_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games.olldbz_java.entity.result.slotDetail.SlotDetailOlldbzFreeGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 歐賴利的寶藏虎機 免費遊戲
public class SlotDetailHlrOlldbzFreeGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendOlldbzFreeGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrOlldbzFreeGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendOlldbzFreeGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailOlldbzFreeGame calculateFreeGameDetailPerRound(RoundHlrResult roundHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 10. 回傳
        return new SlotDetailOlldbzFreeGame(
