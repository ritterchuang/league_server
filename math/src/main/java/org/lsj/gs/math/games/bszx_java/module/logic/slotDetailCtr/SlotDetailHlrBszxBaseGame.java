package org.lsj.gs.math.games.bszx_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games.bszx_java.enity.result.slotDetail.SlotDetailBszxBaseGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 寶石之星虎機 基本遊戲
public class SlotDetailHlrBszxBaseGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendBszxBaseGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrBszxBaseGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendBszxBaseGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailBszxBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 7. 回傳
        return new SlotDetailBszxBaseGame(spinCost, baseGameTotalWin, screenColumn_1, screenColumn_2, screenColumn_3, screenColumn_4, screenColumn_5);
    }
}