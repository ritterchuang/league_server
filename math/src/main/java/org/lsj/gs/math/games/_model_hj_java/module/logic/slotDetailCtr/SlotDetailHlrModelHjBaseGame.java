package org.lsj.gs.math.games._model_hj_java.module.logic.slotDetailCtr;
...
import org.lsj.gs.math.games._model_hj_java.enity.result.slotDetail.SlotDetailModelHjBaseGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 模板虎機 基本遊戲
public class SlotDetailHlrModelHjBaseGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendModelHjBaseGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrModelHjBaseGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendModelHjBaseGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailModelHjBaseGame calculatePureBaseGameDetail(SpinRequest spinRequest, GameStateHlrResult gameStateHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 7. 回傳
        return new SlotDetailModelHjBaseGame(spinCost, baseGameTotalWin, screenColumn_1, screenColumn_2, screenColumn_3, screenColumn_4, screenColumn_5);
    }
}