package org.lsj.gs.math.games.cjwp_java.module.gameStateHlr;
...
import org.lsj.gs.math.games._develop_hj_java.enity.result.gameStateResult.GameStateHlrResultExtendDevelopHjBaseGame;
...
// 超級王牌 基本遊戲處理者
public class GameStateHlrCjwpBaseGame extends AbstractGameStateHlr implements IGameStateHlr {
    private final LogicSlotCascade logic; // 牌桌邏輯
    private final GameStateConfigExtendCjwpBaseGame gameStateConfigExtend; // 遊戲狀態額外設定
    private final RoundCascadeGameConfigExtendCjwpBaseGame roundConfigExtend; // 消除局額外設定

    public GameStateHlrCjwpBaseGame(int conditionId, int gameStateId, ITableUtilSlot tableUtil, ILogicSlot logic, GameStateConfig gameStateConfig) {
        super(conditionId, gameStateId, gameStateConfig, tableUtil);

        this.logic = (LogicSlotCascade) logic;
        this.gameStateConfigExtend = (GameStateConfigExtendCjwpBaseGame) gameStateConfig.getGameStateConfigExtend();
        this.roundConfigExtend = (RoundCascadeGameConfigExtendCjwpBaseGame) ((RoundConfigCascade)gameStateConfig.getRoundConfig()).getRoundCascadeGameConfigExtend();
    }
