...
import org.lsj.gs.math.games.qzpj_java.entity.detail.DetailBet;
import org.lsj.gs.math.games.qzpj_java.entity.message.CtsBetQzpjJava;
import org.lsj.gs.math.games.qzpj_java.entity.message.StcBetQzpjJava;
import org.lsj.gs.math.games.qzpj_java.entity.message.StcBetResultQzpjJava;
import org.lsj.gs.math.games.qzpj_java.entity.message.StcTurnQzpjJava;
...
// 下注狀態 TODO 邏輯修改
public class StateBetQzpjJava extends AbstractStateQzpjJava {
    private static final String STATE_NAME = "bet";
    private static final Logger LOG = LoggerFactory.getLogger(StateBetQzpjJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateBetQzpjJava(TableQzpjJava table, LogicQzpjJava logic, RobotLogicQzpjJava aiLogic, GameAdjustQzpjJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 5. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), ConstQzpjJava.TimeEnumQzpjJava.BET.getCode(), ConstQzpjJava.TimeEnumQzpjJava.BET.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 玩家下注
    public void onMessageBet(int chairIndex, CtsBetQzpjJava data){
    ...
    private void finishBetState(){
        ...
        // 4. 傳送下注結果
        StcBetResultQzpjJava stcBetResult = new StcBetResultQzpjJava(super.logic.getBetResultArray());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetResult);
        super.table.sendMessageToHumanPlayer(stcBetResult);

        // 5. 設置定時器
        super.table.getTableTimer().schedule(new TimerTaskTransState(this), ConstQzpjJava.TimeEnumQzpjJava.BET_RESULT.getMilliTimeSec());
    }
    ...
    private void sendCanBetRate() {
        StcTurnQzpjJava stcTurn = new StcTurnQzpjJava(
                ConstQzpjJava.OperationEnumQzpjJava.BET.getCode(),
                ConstQzpjJava.TimeEnumQzpjJava.BET.getTimeSec(),
    ...
    private void sendBetRate(int chairIndex, int rate) {
        StcBetQzpjJava stcBet = new StcBetQzpjJava(chairIndex, rate);
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateBetQzpjJava state;

        public TimerTaskTimeOut(StateBetQzpjJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransState extends TimerTask {
        ...
        public TimerTaskTransState(StateBetQzpjJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotBet extends TimerTask {
        ...
        public TimerTaskRobotBet(StateBetQzpjJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageBet(this.chairIndex, new CtsBetQzpjJava(this.rate));
        }
    }
}