...
import org.lsj.gs.math.games.qznn_java.entity.ConstQznnJava.OperationEnumQznnJava;
import org.lsj.gs.math.games.qznn_java.entity.ConstQznnJava.TimeEnumQznnJava;
import org.lsj.gs.math.games.qznn_java.entity.detail.DetailBet;
import org.lsj.gs.math.games.qznn_java.entity.message.CtsBetQznnJava;
import org.lsj.gs.math.games.qznn_java.entity.message.StcBetQznnJava;
import org.lsj.gs.math.games.qznn_java.entity.message.StcBetResultQznnJava;
import org.lsj.gs.math.games.qznn_java.entity.message.StcTurnQznnJava;
...
// 下注狀態
public class StateBetQznnJava extends AbstractStateQznnJava {
    private static final String STATE_NAME = "bet";
    private static final Logger LOG = LoggerFactory.getLogger(StateBetQznnJava.class);
    private final FinishLock finishLock = new FinishLock(new ReentrantLock()); // 完成鎖

    public StateBetQznnJava(TableQznnJava table, LogicQznnJava logic, RobotLogicQznnJava aiLogic, GameAdjustQznnJava gameAdjust, int stateId) {
    ...
    public void onEnter() {
        ...
        // 5. 設定定時器
        super.table.getTableTimer().schedule(new TimerTaskTimeOut(this), TimeEnumQznnJava.BET.getCode(), TimeEnumQznnJava.BET.getMilliTimeSec());
    }
    ...
    //* 流程相關 *//
    // 玩家下注
    public void onMessageBet(int chairIndex, CtsBetQznnJava data){
    ...
    private void finishBetState(){
        ...
        // 4. 傳送下注結果
        StcBetResultQznnJava stcBetResultQznnJava = new StcBetResultQznnJava(super.logic.getBetResultArray());
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetResultQznnJava);
        super.table.sendMessageToHumanPlayer(stcBetResultQznnJava);

        // 5. 設置定時器
        super.table.getTableTimer().schedule(new TimerTaskTransState(this), TimeEnumQznnJava.BET_RESULT.getMilliTimeSec());
    }
    ...
    private void sendCanBetRate() {
        StcTurnQznnJava stcTurnQznnJava = new StcTurnQznnJava(
                OperationEnumQznnJava.BET.getCode(),
                TimeEnumQznnJava.BET.getTimeSec(),
                0,
    ...
    private void sendBetRate(int chairIndex, int rate) {
        StcBetQznnJava stcBetQznnJava = new StcBetQznnJava(chairIndex, rate);
        LOG.info(super.table.getTableLogTitle() + " message: {}", stcBetQznnJava);
        super.table.sendMessageToHumanPlayer(stcBetQznnJava);
    }
    ...
    public class TimerTaskTimeOut extends TimerTask {
        private final StateBetQznnJava state;

        public TimerTaskTimeOut(StateBetQznnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskTransState extends TimerTask {
        ...
        public TimerTaskTransState(StateBetQznnJava state){
            this.state = state;
        }
    ...
    public class TimerTaskRobotBet extends TimerTask {
        ...
        public TimerTaskRobotBet(StateBetQznnJava state, int chair, int rate){
            this.state = state;
        ...
        @Override
        public void run() {
            this.state.onMessageBet(this.chairIndex, new CtsBetQznnJava(this.rate));
        }
    }
}