...
import org.lsj.gs.math.games.zjh_java.entity.PreGameAdjustResultZjhJava;
...
// 炸金花發牌
public class ShuffleZjhJava extends AbstractShuffle implements IShuffle {
    private final LogicZjhJava logic; // 邏輯
    private final int DEAL_COUNT_PER_PLAYER = 5; // 每人發牌數

    public ShuffleZjhJava(AlgorithmTypeCtr algorithmTypeCtr, LogicZjhJava logicQZNN, ITableUtil tableUtil) {
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 5. 封裝
        return new PreGameAdjustResultZjhJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultZjhJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
    ...
    public GameAdjustResult calculateGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 計算所有玩家輸贏結果
        Map<Integer, UidScore> uidScoreMap = this.logic.calculateUidScoreMap(((PreGameAdjustResultZjhJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap());
