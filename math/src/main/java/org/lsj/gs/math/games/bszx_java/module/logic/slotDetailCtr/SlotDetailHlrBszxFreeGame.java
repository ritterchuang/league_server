...
import org.lsj.gs.math.games.bszx_java.enity.result.slotDetail.SlotDetailBszxFreeGame;

import java.util.ArrayList;
import java.util.List;

// 虎機詳細記錄計算者 寶石之星虎機免費遊戲
public class SlotDetailHlrBszxFreeGame extends AbstractSlotDetailHlr implements ISlotDetailHlr {
    private final ConstMathSlot.SlotDetailType slotDetailType; // 虎機詳細記錄類型
    private final SlotDetailHlrConfigExtendBszxFreeGame extendConfig; // 虎機詳細記錄額外設定

    public SlotDetailHlrBszxFreeGame(SlotDetailHlrConfig slotDetailHlrConfig, ITableUtil tableUtil1) {
        super(slotDetailHlrConfig, tableUtil1);
        this.slotDetailType = slotDetailHlrConfig.getSlotDetailType();
        this.extendConfig = (SlotDetailHlrConfigExtendBszxFreeGame) slotDetailHlrConfig.getSlotDetailHlrConfigExtend();
    }
    ...
    // 計算基礎遊戲詳細記錄
    private SlotDetailBszxFreeGame calculateFreeGameDetailPerRound(RoundHlrResult roundHlrResult) {
        // 1. 取得轉型局結果
        ...
        // 10. 回傳
        return new SlotDetailBszxFreeGame(
