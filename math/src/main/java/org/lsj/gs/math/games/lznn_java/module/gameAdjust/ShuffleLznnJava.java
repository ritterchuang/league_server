...
import org.lsj.gs.math.games.lznn_java.entity.PreGameAdjustResultLznnJava;
...
// 賴子牛牛發牌
public class ShuffleLznnJava extends AbstractShuffle implements IShuffle {
    private final LogicLzNnJava logic; // 邏輯
    private final int DEAL_COUNT_PER_PLAYER = 5; // 每人發牌數

    public ShuffleLznnJava(AlgorithmTypeCtr algorithmTypeCtr, LogicLzNnJava logicQZNN, ITableUtil tableUtil) {
    ...
    private PreGameAdjustResult dealPreGameResult(){
        ...
        // 5. 封裝
        return new PreGameAdjustResultLznnJava(unTakenCardListMap);
    }
    ...
    public void applyPreGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 將牌堆得牌改成取出
        Map<Integer, List<ICard>> unTakenCardListMap = ((PreGameAdjustResultLznnJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap();
    ...
    public GameAdjustResult calculateGameAdjustResult(PreGameAdjustResult preGameAdjustResult) {
        // 1. 計算所有玩家輸贏結果
        Map<Integer, UidScore> uidScoreMap = this.logic.calculateUidScoreMap(((PreGameAdjustResultLznnJava) preGameAdjustResult).getAllPlayerUnTakenCardListMap());
